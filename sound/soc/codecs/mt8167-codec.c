...
static int mt8167_codec_mute(struct snd_soc_dai *dai, int mute)
{
	if (mute)
	{
		gpio_direction_output(gpio_amp_mute_pin, 0);
    }
	else
	{
		msleep(300);
		gpio_direction_output(gpio_amp_mute_pin, 1);
	}
...
static const struct snd_soc_dai_ops mt8167_codec_aif_dai_ops = {
	...
	.digital_mute	= mt8167_codec_mute,
};
...
static int mt8167_codec_dev_probe(struct platform_device *pdev)
{
	...
	gpio_amp_mute_pin = of_get_named_gpio(dev->of_node, "spk-mute", 0);
    if(gpio_amp_mute_pin < 0)
     {
          dev_dbg(dev, "%s gpio_amp_mute_pin error\n", __func__);
     }
     else
     {
          gpio_request(gpio_amp_mute_pin, NULL);
          gpio_direction_output(gpio_amp_mute_pin, 0);
     }
