...
static int initial_vcnl4200(struct vcnl4200_info *lpi);
...
static int get_ls_adc_value(uint16_t *als_step, bool resume)
{
	...
	/* Read ALS data: */
	ret = _vcnl4200_I2C_Read_Word(lpi->slave_addr, ALS_DATA, als_step);
	if (ret < 0) {
		pr_err(
...
static void ls_initial_cmd(struct vcnl4200_info *lpi)
{	
...
static int initial_vcnl4200(struct vcnl4200_info *lpi)
{
	...
	val = gpio_get_value(lpi->intr_pin);
	D("[PS][VCNL4200] %s, INTERRUPT GPIO val = %d\n", __func__, val);
...
static int vcnl4200_setup(struct vcnl4200_info *lpi)
{
	...
	ret = initial_vcnl4200(lpi);
	if (ret < 0) {
		pr_err(
			"[PS_ERR][VCNL4200 error]%s: fail to initial vcnl4200 (%d)\n",
			__func__, ret);
		goto fail_free_intr_pin;
	}
	...
	const struct i2c_device_id *id)
{
  ...
  lpi->intr_pin = pdata->intr;
    ...
    // disable sensor
   // lightsensor_enable(lpi); // yang
   // psensor_enable(lpi); // yang
