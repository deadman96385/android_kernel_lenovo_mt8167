...
static int typec_pltfrm_probe(struct platform_device *pdev)
{
	...
	if (res == NULL) {
		dev_err(dev, "platform_get_resource Intialization failed\n");
		return -ENODEV;
	}
	...
	if (irq <= 0) {
		dev_err(dev, "irq_create_mapping Intialization failed\n");
		return -EINVAL;
	}
	...
	if (err) {
		dev_err(dev, "typec_init Intialization failed\n");
    ...
    dev_info(dev, "typec node: %s\n", dev_driver_string(hba->dev));
	mt6392_typec_init_debugfs(hba);
#endif
