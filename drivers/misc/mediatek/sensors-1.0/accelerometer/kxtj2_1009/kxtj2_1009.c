...
//#define KXTJ2_ENABLE_WUFE
#ifdef KXTJ2_ENABLE_WUFE
...
#define KXTJ2_1009_DEV_NAME        "gsensor_kxtj2_1009"
/*----------------------------------------------------------------------------*/
...
struct kxtj2_1009_i2c_data {
    ...
    struct work_struct irq_wufe_work;
#endif
};
/*----------------------------------------------------------------------------*/
#ifdef CONFIG_OF
...
#ifdef KXTJ2_ENABLE_WUFE
                        if(atomic_read(&obj_i2c_data->en_sig_motion) == 0) { 
                            err = KXTJ2_1009_SetPowerMode(obj_i2c_data->client, enable_status);
                        } else {
                            GSE_LOG("kxjt2 en_sig_motion active\n");
                        }
#else
                        err = KXTJ2_1009_SetPowerMode(obj_i2c_data->client, enable_status);
#endif
...
#define KXTJ2_CTRL_REG1_WUFE 0x2

#define KXTJ2_CTRL_REG2 0x1D
/* Wakeup ODR 25Hz */
#define KXTJ2_OWUF_VAL 0x5

#define KXTJ2_INT_CTRL_REG1 0x1E
#define KXTJ2_INT_CTRL_REG1_IEN 0x20
#define KXTJ2_INT_CTRL_REG1_IEA 0x10
#define KXTJ2_INT_CTRL_REG1_IEL 0x8

#define KXTJ2_INT_CTRL_REG2 0x1F
#define KXTJ2_INT_CTRL_REG2_ALL_AXIS 0x3F
#define KXTJ2_INT_CTRL_REG2_UNLATCH 0x80

#define KXTJ3_WAKEUP_COUNTER 0x29
#define KXTJ3_WUFC_VAL 25

#define KXTJ3_WAKEUP_THRESHOLD_H 0x6A
#define KXTJ3_WAKEUP_THRESHOLD_L 0x6B
/* 12bit value, 3.9mg/count, 1000mg/3.9mg=256counts */
#define KXTJ3_WUTH_VAL 256

#define KXTJ2_INT_SOURCE1 0x16
#define KXTJ2_INT_SOURCE1_WUFS 0x2

#define KXTJ2_INT_REL 0x1A

static int kxtj2_enable_wufe(struct i2c_client *client)
{
	int err;
	u8 ctrl1_reg;
	u8 write_buf;

	/* read ctrl1 and set to standby mode */
	err = hwmsen_read_byte(client, KXTJ2_CTRL_REG1, &ctrl1_reg);
	if (err) {
		return err;
	}

	write_buf = ctrl1_reg & 0x7F;
	err = hwmsen_write_byte(client, KXTJ2_CTRL_REG1, write_buf);
	if (err) {
		return err;
	}

	/* set sensor wakeup ODR */
	err = hwmsen_write_byte(client, KXTJ2_CTRL_REG2, KXTJ2_OWUF_VAL);
	if (err) {
		return err;
	}

	/* set sensor wakeup counter */
	err = hwmsen_write_byte(client, KXTJ3_WAKEUP_COUNTER, KXTJ3_WUFC_VAL);
	if (err) {
		return err;
	}
	
	/* set sensor wakeup threshold high and low */
	write_buf = KXTJ3_WUTH_VAL >> 4;
	GSE_LOG("KXTJ3_WAKEUP_THRESHOLD_H 0x%x\n", write_buf);
	err = hwmsen_write_byte(client, KXTJ3_WAKEUP_THRESHOLD_H,
		write_buf);
	if (err) {
		return err;
	}

	write_buf = 0x0F0 & (KXTJ3_WUTH_VAL << 4);
	GSE_LOG("KXTJ3_WAKEUP_THRESHOLD_L 0x%x\n", write_buf);
	err = hwmsen_write_byte(client, KXTJ3_WAKEUP_THRESHOLD_L,
		write_buf);
	if (err) {
		return err;
	}
	...
	write_buf = KXTJ2_INT_CTRL_REG1_IEN; /* enable */
	/* active low, no need to set bit */ 
	/* latch, no ned to set bit */
	GSE_LOG("KXTJ2_INT_CTRL_REG1 0x%x\n", write_buf);
	err = hwmsen_write_byte(client, KXTJ2_INT_CTRL_REG1, write_buf);
	if (err) {
		return err;
	}

	/* set wufe interrupt configuration */
	write_buf = KXTJ2_INT_CTRL_REG2_ALL_AXIS;
	GSE_LOG("KXTJ2_INT_CTRL_REG2 0x%x\n", write_buf);
	err = hwmsen_write_byte(client, KXTJ2_INT_CTRL_REG2, write_buf);
	if (err) {
		return err;
	}

	/* enable wufe */
	write_buf = KXTJ2_CTRL_REG1_WUFE | (ctrl1_reg & 0x7F);
	err = hwmsen_write_byte(client,KXTJ2_CTRL_REG1, write_buf);
	if (err) {
		return err;
	}
	...
	write_buf = KXTJ2_CTRL_REG1_WUFE | ctrl1_reg;
	err = hwmsen_write_byte(client,KXTJ2_CTRL_REG1, write_buf);
	if (err) {
		return err;
	}

	GSE_LOG("kxtj2_enable_wufe - ctrl1_reg 0x%x\n", write_buf);

	return 0;
}

static int kxtj2_disable_wufe(struct i2c_client *client)
{
	int err;
	u8 ctrl1_reg;
	u8 write_buf;
	...
	/* make sure WUFS interrupt bit is cleared */
	(void)hwmsen_read_byte(client, KXTJ2_INT_REL, &dummy);
	...
	GSE_LOG("kxtj2_disable_wufe - ctrl1_reg 0x%x\n", write_buf);

	return 0;
}


static void kxtj2_irq_wufe_work_func(struct work_struct *work)
{
	struct kxtj2_1009_i2c_data *obj = container_of(work,
			struct kxtj2_1009_i2c_data, irq_wufe_work);
	int err;
	u8 int_src1;
	u8 dummy;

	/* read interrupt status */
	err = hwmsen_read_byte(obj->client, KXTJ2_INT_SOURCE1, &int_src1);
	if (err) {
		return;
	}

	//GSE_LOG("kxtj2 int_src1 0x%x\n", int_src1);

	if (int_src1 & KXTJ2_INT_SOURCE1_WUFS) {
	        GSE_LOG("kxtj2 KXTJ2_INT_SOURCE1_WUFS 0x%x\n", int_src1);
		if (atomic_read(&obj->en_sig_motion) == 1) {
			step_notify(TYPE_SIGNIFICANT);
		}
	}

	/* clear interrupt */
	(void)hwmsen_read_byte(obj->client, KXTJ2_INT_REL, &dummy);

	return;
}

static irqreturn_t kxtj2_eint_handler(int irq, void *desc)
{
	if (obj_i2c_data == NULL)
		return IRQ_HANDLED;
	if (obj_i2c_data->client == NULL)
		return IRQ_HANDLED;

	schedule_work(&obj_i2c_data->irq_wufe_work);

	return IRQ_HANDLED;
}

static int kxtj2_request_irq(struct kxtj2_1009_i2c_data *obj)
{
	int err;

	/* TODO: change to correct interrupt */
	obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek,fpc1145");

	if(!obj->irq_node) {
	        GSE_ERR("irq_node not found\n");
		return 0;
	}

	if(obj->irq_node && obj->irq_node->name) {
	        GSE_LOG("kxtj2_request_irq %s\n", obj->irq_node->name);
	}
	...
	err = request_irq(obj->irq, kxtj2_eint_handler, IRQF_TRIGGER_FALLING, "kxtj3-int", NULL);
	if (err) {
		GSE_ERR("kxtj2 request_irq err=%d\n", err);
		return -EINVAL;
	}
...
static int kxtj2_set_en_sig_motion(struct kxtj2_1009_i2c_data *obj, int en)
{
	int err = 0;

	en = (en >= 1) ? 1 : 0;  /* set sig motion sensor status */

	if (atomic_read(&obj->en_sig_motion) != en) {

		/* enable/disable sensor wakeup functionality */
		if (en) {
			err = kxtj2_enable_wufe(obj->client);
		} else  {
			err = kxtj2_disable_wufe(obj->client);
		}
		atomic_set(&obj->en_sig_motion, en);
	}
...
static int kxtj2_step_c_open_report_data(int open)
{
	return 0;
}

static int kxtj2_step_c_set_delay(u64 delay)
{
	return 0;
}
static int kxtj2_step_c_enable_nodata(int en)
{
	return 0;
}

static int kxtj2_step_c_enable_significant(int en)
{
	int err = 0;

	if (obj_i2c_data == NULL) {
		GSE_ERR("kxtj2 obj_i2c_data == NULL");
		return 0;
	}

	GSE_LOG("kxtj2_step_c_enable_significant en = %d\n", en);

	mutex_lock(&kxtj2_1009_mutex);
	if (en == 1) {
		err = KXTJ2_1009_SetPowerMode(obj_i2c_data->client, true);
	}
	...
	/* enable/disable sig motion */
	err = kxtj2_set_en_sig_motion(obj_i2c_data, en);
...
static int kxtj2_step_c_enable_step_detect(int enable)
{
	return 0;
}

static int kxtj2_step_c_get_data(uint32_t *value, int *status)
{
	return 0;
}

static int kxtj2_1009_stc_get_data_significant(uint32_t *value, int *status)
{
	return 0;
}

static int kxtj2_1009_stc_get_data_step_d(uint32_t *value, int *status)
{
	return 0;
}

static int kxtj2_step_d_set_delay(u64 delay)
{
	return 0;
}

static int kxtj2_step_c_probe(void)
{
	struct step_c_control_path ctl = {0};
	struct step_c_data_path data = {0};
	int err = 0;

	ctl.open_report_data = kxtj2_step_c_open_report_data;
	ctl.enable_nodata = kxtj2_step_c_enable_nodata;
	ctl.enable_step_detect = kxtj2_step_c_enable_step_detect;
	ctl.enable_significant = kxtj2_step_c_enable_significant;
	ctl.step_c_set_delay = kxtj2_step_c_set_delay;
	ctl.is_report_input_direct = false;
	ctl.step_d_set_delay = kxtj2_step_d_set_delay;
	...
	if (err) {
		GSE_ERR("control_path err = %d\n", err);
		goto exit;
	}
	data.get_data = kxtj2_step_c_get_data;
	data.vender_div = 1000;
	data.get_data_significant = kxtj2_1009_stc_get_data_significant;
	data.get_data_step_d = kxtj2_1009_stc_get_data_step_d;
	err = step_c_register_data_path(&data);
	if (err) {
		GSE_ERR("data_path err = %d\n", err);
		goto exit;
	}
	printk("%s: OK\n", __func__);
	return 0;
...
static int kxtj2_1009_stc_remove(void)
{
	GSE_FUN();
	return 0;
}

static int kxtj2_1009_stc_local_init(void)
{
	GSE_FUN();

	if (kxtj2_step_c_probe()) {
		GSE_ERR("failed to register kxtj2_1009 step_c driver\n");
		return -ENODEV;
	}
	return 0;
}

static struct step_c_init_info kxtj2_1009_stc_init_info = {
	.name = "kxtj2_step_counter",
	.init = kxtj2_1009_stc_local_init,
	.uninit = kxtj2_1009_stc_remove,
};
#endif
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        ...
        GSE_ERR("client->addr 0x%x\n", client->addr);
       //client->addr = 0x0f;
      //GSE_ERR("client->addr22 0x%x\n", client->addr);
      err = get_accel_dts_func(client->dev.of_node, hw);
        if (err) {
                GSE_ERR("get_accel_dts_func err=%d\n", err);
                goto exit;
        }
	...
	err = kxtj2_request_irq(obj);
	if (err) {
		goto exit;
	}
	//enable_irq_wake(obj->irq);
	atomic_set(&obj->en_sig_motion, 0);
	INIT_WORK(&obj->irq_wufe_work, kxtj2_irq_wufe_work_func);
#endif
...
#ifdef KXTJ2_ENABLE_WUFE
	step_c_driver_add(&kxtj2_1009_stc_init_info);
#endif
        printk("kxtj2_1009_init---\n");
        return 0;
}
