...
struct typec_hba {
	...
	struct pinctrl_state *typec_vbus_monitor;
	unsigned int typec_vbus_monitor_pin;
#endif
...
static inline uint16_t typec_readw(struct typec_hba *hba, unsigned int reg)
{
	...
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_3)
		dev_info(hba->dev,
			 "+%s reg:0x%x, hba->regmap:0x%p\n",
			 __func__, reg, hba->regmap);

	regmap_read(hba->regmap, (hba->addr_base + reg), &rdata);
	ui2_data = (uint16_t) rdata;
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_3)
		dev_info(hba->dev,
			 "-%s,rdata:0x%x ui_data:0x%x\n",
			 __func__, rdata, ui2_data);
...
static inline void typec_writew(struct typec_hba *hba, uint16_t val, unsigned int reg)
{
	...
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_3) {
		dev_info(hba->dev,
			 "+%s, reg:0x%x rdata:0x%x, val:0x%x\n",
			 __func__, reg, rdata, val);
	}

	regmap_write(hba->regmap, (hba->addr_base + reg), val);
	regmap_read(hba->regmap, (hba->addr_base + reg), &rdata);
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_3)
		dev_info(hba->dev, "-%s, value:0x%x\n", __func__, rdata);
}
