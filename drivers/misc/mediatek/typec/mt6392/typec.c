...
void typec_vbus_present(struct typec_hba *hba, uint8_t enable)
{
	...
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_1 && (hba->vbus_present != enable))
		dev_info(hba->dev, "VBUS_PRESENT %s", ((enable) ? "ON" : "OFF"));
...
{
	if (hba->dbg_lvl >= TYPEC_DBG_LVL_1 && (hba->vbus_det_en != enable))
		dev_info(hba->dev, "VBUS_DET %s", ((enable) ? "ON" : "OFF"));
...
void typec_drive_vbus(struct typec_hba *hba, uint8_t on)
{
	...
	if ((hba->dbg_lvl >= TYPEC_DBG_LVL_1) && (hba->vbus_en != on))
		dev_info(hba->dev, "VBUS %s", (on ? "ON" : "OFF"));
...
int typec_is_vbus_present(struct typec_hba *hba)
{
	int vbus_exist = 0;

	vbus_exist = !gpio_get_value(hba->typec_vbus_monitor_pin); //modify by kxz
...
int typec_is_vbus_present(struct typec_hba *hba, enum enum_vbus_lvl lvl)
{
	...
	/* Vdct monitor vbus need multi 10 */
	tmp = PMIC_IMM_GetOneChannelValue(2, 2, 1) * 10;

	if (hba->dbg_lvl >= TYPEC_DBG_LVL_2)
		dev_info(hba->dev, "%s:PMIC tmp:%dmV\n", __func__, tmp);

	if (((lvl == TYPEC_VSAFE_5V) && (tmp >= 4800))
	    || ((lvl == TYPEC_VSAFE_0V) && (tmp < 800)))
		vbus_exist = 1;
	else
		dev_info(hba->dev, "%s:PMIC tmp:%dmV(regression)\n", __func__, tmp);

	return vbus_exist;

}
#endif
static int typec_drive_vbus_init(struct typec_hba *hba)
{
	int ret = 0;
#if IS_ENABLED(CONFIG_TCPC_MT6392_VBUS_GPIO)
	...
	if (IS_ERR(hba->pinctrl)) {
		ret = IS_ERR(hba->pinctrl);
		dev_err(hba->dev, "Cannot find typec pinctrl since %i!\n", ret);
		goto exit;
	}

#if IS_ENABLED(CONFIG_TCPC_MT6392_VBUS_CTRL)
	hba->typec_drvvbus = pinctrl_lookup_state(hba->pinctrl, "drvvbus_init");
	if (IS_ERR(hba->typec_drvvbus)) {
		ret = PTR_ERR(hba->typec_drvvbus);
		dev_err(hba->dev, "Cannot find typec pinctrl drvvbus_init since %i!\n", ret);
		goto error;
	}
	...
	if (IS_ERR(hba->typec_drvvbus_low)) {
		...
		dev_err(hba->dev, "Cannot find typecpinctrl drvvbus_low since %i!\n", ret);
		goto error;
	}
	...
	if (IS_ERR(hba->typec_drvvbus_high)) {
		...
		dev_err(hba->dev, "Cannot find typec pinctrl drvvbus_high since %i!\n", ret);
		goto error;
	}

	ret = pinctrl_select_state(hba->pinctrl, hba->typec_drvvbus);
	if (ret < 0) {
		goto error;
		dev_err(hba->dev, "pinctrl_select_state typec_drvvbus failure since %i!\n", ret);
		goto error;
	}
#endif

#if IS_ENABLED(CONFIG_TCPC_MT6392_VBUS_GPIO)
	hba->typec_vbus_monitor = pinctrl_lookup_state(hba->pinctrl, "typec_vbus_monitor_init");
	if (IS_ERR(hba->typec_vbus_monitor)) {
		ret = PTR_ERR(hba->typec_vbus_monitor);
		dev_err(hba->dev,
			"Cannot find typec pinctrl typec_vbus_monitor_init since %i!\n", ret);
		goto error;
	}

	ret = pinctrl_select_state(hba->pinctrl, hba->typec_vbus_monitor);
	if (ret < 0)
		goto error;

	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6392-typec");
	if (node != NULL) {
	    hba->typec_vbus_monitor_pin = of_get_named_gpio(node, "typec_vbus_monitor_gpio", 0);
		if (!gpio_is_valid(hba->typec_vbus_monitor_pin))
			dev_err(hba->dev,
				"get dtsi typec_vbus_monitor_gpio failure since %i\n",
				hba->typec_vbus_monitor_pin);

		dev_info(hba->dev, "====> typec_vbus_monitor_gpio virtual Number %u!!!\n",
			 hba->typec_vbus_monitor_pin);
    }
#endif
	return ret;
error:
	devm_pinctrl_put(hba->pinctrl);
...
static void typec_wait_vbus_on_try_wait_snk(struct work_struct *work)
{
	...
	while (((typec_readw(hba, TYPE_C_CC_STATUS) & RO_TYPE_C_CC_ST) == TYPEC_STATE_TRY_WAIT_SNK)
	       && (i < POLLING_MAX_TIME)) {
#if IS_ENABLED(CONFIG_TCPC_MT6392_VBUS_GPIO)
		if (hba->vbus_det_en && typec_is_vbus_present(hba)) {
			typec_vbus_present(hba, 1);
			dev_info(hba->dev, "Vbus ON in TryWait.SNK state\n");
			break;
		}
#else
...
static void typec_wait_vbus_on_attach_wait_snk(struct work_struct *work)
{
	...
	while (((typec_readw(hba, TYPE_C_CC_STATUS) & RO_TYPE_C_CC_ST) ==
		...
		if (hba->vbus_det_en && typec_is_vbus_present(hba)) {
			typec_vbus_present(hba, 1);
			dev_info(hba->dev, "Vbus ON in AttachWait.SNK state\n");
			break;
		}
#else
		if (hba->vbus_det_en && typec_is_vbus_present(hba, TYPEC_VSAFE_5V)) {
...
static void typec_wait_vbus_off_attached_snk(struct work_struct *work)
{
	...
	while ((typec_readw(hba, TYPE_C_CC_STATUS) & RO_TYPE_C_CC_ST) == TYPEC_STATE_ATTACHED_SNK) {
		...
		if (hba->vbus_det_en && !typec_is_vbus_present(hba)) {
			...
			dev_info(hba->dev, "Vbus OFF in Attachd.SNK state\n");
			break;
		}
#else
		if (hba->vbus_det_en && !typec_is_vbus_present(hba, TYPEC_VSAFE_5V)) {
...
static void typec_wait_vbus_off_then_drive_attached_src(struct work_struct *work)
{
	...
	while ((typec_readw(hba, TYPE_C_CC_STATUS) & RO_TYPE_C_CC_ST) == TYPEC_STATE_ATTACHED_SRC) {
		...
		if (hba->vbus_det_en && !typec_is_vbus_present(hba)) {
			typec_drive_vbus(hba, 1);
			dev_info(hba->dev, "vBus ON in Attached.SRC state\n");
			break;
		}
#else
	       ...
	       unsigned int irq, int id)
{
	...
	/* initialize TYPEC */
	regmap_read(hba->regmap, 0x10E, &reg_value);
	reg_value &= ~(0x03L << 8);
	regmap_write(hba->regmap, 0x10E, reg_value);
	...
	dev_info(hba->dev, "  ==> role: %i\n", hba->support_role);
	dev_info(hba->dev, "  ==> Rp: %i\n", hba->rp_val);
	dev_info(hba->dev, "  ==> dbg_lvl: %i\n", hba->dbg_lvl);
#if IS_ENABLED(CONFIG_TCPC_MT6392_VBUS_GPIO)
	dev_info(hba->dev, "  ==> GPIO vBus monitor: ON\n");
#else
	dev_info(hba->dev, "  ==> VCDT vBus monitor: ON\n");
#endif
