...
{
#if IS_ENABLED(CONFIG_TCPC_MT6392) || (!IS_ENABLED(CONFIG_MTK_MUSB_SW_WITCH_MODE))
	if (!mtk_musb) {
		DBG(0, "mtk_musb = NULL\n");
		return;
	}
...
#if IS_ENABLED(CONFIG_TCPC_MT6392)
	vbus_exist = musb_typec_sink;
	DBG(0, "typec make it sync musb_typec_sink vBus %s\n", vbus_exist ? "EXIST" : "NOT EXIST");
#else
